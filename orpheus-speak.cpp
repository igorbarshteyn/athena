...
static bool speak_text(const Config &cfg, SnacDecoder &snac, const std::string &text) {
    ...
    if (use_chunked) {
        ...
        // requests in flight at once.  With -np 2, this means 2 slots
        ...
        static constexpr int max_inflight = 4;  // np=2 active + 2 queued
...
int main(int argc, char **argv) {
    ...
    if (!cfg.watch_file.empty()) {
        // ── Watch mode: stay alive, process file on each change ──────────
        // Uses inotify instead of polling — reacts instantly when
        // speak-daemon.sh atomically mv's the trigger file into place.
        // Zero CPU cost while idle (blocks on read() instead of sleep).
        ...
        // Extract directory and basename from watch path for inotify.
        // inotify must watch the DIRECTORY because speak-daemon.sh uses
        // mv (rename), which replaces the inode — a watch on the file
        // itself would follow the old inode after rename.
        std::string watch_dir, watch_base;
        {
            // Use copies since dirname/basename may modify input
        ...
        // IN_MOVED_TO: catches mv (atomic rename from speak-daemon.sh)
        // IN_CLOSE_WRITE: catches direct write-and-close (fallback)
        ...
        // Event buffer — sized for several events with filenames
        char evbuf[4096] __attribute__((aligned(__alignof__(struct inotify_event))));

        while (true) {
            // Block until filesystem event (zero CPU while idle)
            int len = read(ifd, evbuf, sizeof(evbuf));
            if (len <= 0) break;  // error or fd closed

            // Scan events for our trigger file
            ...
            if (triggered) {
                ...
                // Signal completion (for speak-daemon.sh synchronization)
