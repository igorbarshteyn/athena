...
int main(int argc, char **argv) {
    ...
    if (!cfg.watch_file.empty()) {
        ...
        while (true) {
            ...
            // Minimum decoded chunks to buffer before starting playback.
            // Trades ~10s extra first-audio latency for gapless audio
            // thereafter.  For single-chunk responses, plays immediately.
            static constexpr int MIN_BUFFER = 3;

            while (true) {
                // ── FILL: collect + decode until buffer ready ──────────
                while (accum_chunks < MIN_BUFFER) {
                    read_new_lines();
                    if (next_collect >= chunks.size() && !pending_chunk.empty())
                        flush_pending();
                    submit_available();
